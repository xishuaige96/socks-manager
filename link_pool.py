...
import json
import os
from router import buildLink0, deleteLink0, ShowToUserException, getUnlivedNodeSet
from router import deleteLink as router_deleteLink
import threading
import shutil
import traceback
...
def allocateForLinkMethod(subscriptionName: str, linkMethodId: str, linkNum: int):
    ...
    unlived_node = getUnlivedNodeSet()
    if target in unlived_node:
        linkNum = 0
    for i in range(linkNum):
        try:
            link_with_client_config = buildLink0(
            ...
            # raise e
            return
        links.append(link_with_client_config)
...
def dequeueFromLinkMethod(subscriptionName: str, linkMethodId: str, linkNum: int, shouldDelete=False) -> list:
    '''
    从一个链路方法删除一个链路，并返回它的client_config。
    subscriptionName和linkMethodId用于唯一标识该链路方法。
    linkNum为需要删除的链路数量。链路会从队首开始删。linkNum为-1时表示删除所有链路

    返回list，list里面是一个个字典，字典里面是client_config和link。
    '''
    links = read_linkPool(subscriptionName, linkMethodId)
    ...
    for i in range(linkNum):
        try:
            link_with_client_config = links[i]
            link = link_with_client_config['link']
            if shouldDelete:
                deleteLink0(link)
            ret.append(link_with_client_config)
        except Exception as e:
...
def getLink(subscriptionName: str, linkMethodId: str, username: str):
    ...
    return: 字典，表示client_config。同时会往userlinks/{username}.json文件中写入{'client_config':{...},'link':[...]}
    '''
    # 读取用户链路文件
    ...
    try:
        ...
        raise ShowToUserException('链路池为空，可能是指定ip节点已失效。请联系管理员。')
    link_with_client_config = configAndLinks[0]
...
def deleteLink(username: str, client_config: dict = None):
    """
    使用link_pool.py的backgroundJobManager，调用router.py的deleteLink
    """
    backgroundJobManager.addJob(router_deleteLink, (username, client_config))
