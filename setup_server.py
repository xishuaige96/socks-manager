...
zipPath = './setup_server/program_package.zip'
scriptPath = './setup_server/setup_server.sh'
script2Path = './setup_server/setup_server2.sh'
clearScriptPath='./setup_server/clear.sh'
zhongkong_ip='49.232.164.17'
...
def setup_environment(hostname, port: int, username, password):
    ...
    connector.SFTP_put(zipPath, remote_user_dir + zipPath.split('/')[-1])
    connector.SFTP_put(scriptPath, remote_user_dir + scriptPath.split('/')[-1])
    connector.SFTP_put(script2Path, remote_user_dir + script2Path.split('/')[-1])
    connector.SFTP_put(clearScriptPath, remote_user_dir + clearScriptPath.split('/')[-1])
    connector.close_SFTP()

    print('传输文件成功，开始执行setup_server.sh脚本')

    output, exit_status = connector.run(
        f'bash {remote_user_dir}/setup_server.sh \'{password}\' \'{zhongkong_ip}\'')
    # 检查执行.sh脚本结束后返回的状态码
    if exit_status == 1:
...
def setup_server(ip):
    ...
    backgroundJobManager.addJob(setup_server0, args=(ip,))
