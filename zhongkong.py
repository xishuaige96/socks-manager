...
from router import buildLink, resetAllBob, closeBob, setBob_randomly, getCountryFlag, delete_Bob, ShowToUserException
...
from link_pool import onSubscriptionCreated, onSubscriptionDeleted, getLink, deleteLink
...
max_used_links_count_limit = 9999999999
...
def heartbeat():
    ...
    query = "SELECT ip FROM linknodelist"
    cursor.execute(query)
    result = cursor.fetchall()
    cursor.close()
    connection.close()
    if (node_ip,) not in result:
        print(f'Unauthorized node {node_ip} tried to send heartbeat')
        return '', 403
...
def run_getLink():
    ...
    except ShowToUserException as e:
...
def run_deleteLink():
    ...
    username = data.get('username', None)
    client_config = data.get('link', None)

    if username is None:
        return jsonify({"message": "Incorrect parameters in request"}), 400

    try:
        deleteLink(username, client_config)
        ...
        if client_config is None:
            set_used_links_count(connection_pool.get_connection(), username, 0)
        else:
            add_used_links_count(connection_pool.get_connection(), connection_pool.get_connection(), username, -1)
    except Exception as e:
...
def run_addUserPermission():
    ...
    try:
        ...
        addUserPermission(connection1, connection2, userList, subscriptionList)
    except Exception as e:
