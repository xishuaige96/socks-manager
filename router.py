...
def read_node_configs():
    '''
    从数据库nodeconfig表中读取数据存到node_configs全局变量中
    node_configs格式见全局变量的下面
    （注：相对于dev2，node_configs不再有bob的参数，只有3个ssh参数。
    ...
    for key in node_configs.keys():
        node_configs[key] = (
            node_configs[key][0],  # ssh端口
            node_configs[key][1],  # ssh用户名
            decrypt(node_configs[key][2])  # ssh密码
        )
...
#     'chacha20',  # 本节点加密方式
#     '123456',  # 本节点密码
#     '49.232.253.70',  # 下一节点ip
#     '1081',  # 下一节点端口
#     'salsa20',  # 下一节点加密方式
#     '654321',  # [10]下一节点密码
# ]
def create_nodeProxyProgram(config):
    '''
    config格式见generate_nodeProxyProgram_configs上面
    所有项均为字符串
    '''
    path = '/root' if config[2] == 'root' else f'/home/{config[2]}'
    commands = [
        f'screen -X -S proxyProgram{config[4]} quit',  # 保险起见，先关闭可能存在的screen
        f'screen -dmS proxyProgram{config[4]}',  # 覆盖创建、创建后不进入screen内
        f'screen -S proxyProgram{config[4]} -X stuff "cd ~/program_package\n"',
        f'screen -S proxyProgram{config[4]} -X stuff "python3 set_config.py {config[4]} {config[5]} {config[6]} {config[7]} {config[8]} {config[9]} {config[10]}\n"',
        f'screen -S proxyProgram{config[4]} -X stuff "./main -j config{config[4]}.json\n"',
        f'(sleep 10 && rm ~/program_package/config{config[4]}.json) &',

        # f'screen -dmS proxyProgram{config[4]}remove',
        # # f'screen -S proxyProgram{config[4]}remove -X stuff "sleep 10\n"',
        # f'screen -S proxyProgram{config[4]}remove -X stuff "rm -f ~/program_package/config{config[4]}.json\n"',
        # f'screen -X -S proxyProgram{config[4]}remove quit',
    ]
...
def destroy_nodeProxyProgram(config):
    '''
    参数为ssh（4个值）+该nodeProxyProgram的端口（[4]）
    '''
    print(f'destroying proxyProgram for (ip:{config[0]}, port:{config[4]})')
    commands = [
        f'screen -X -S proxyProgram{config[4]} quit',
        f'lsof -ti :{config[4]} | xargs kill'
    ]
    run_commands_on_remote_host(config[0], int(
        config[1]), config[2], config[3], commands)
...
def create_nodeProxyPrograms(configs):
    '''
    configs格式见generate_nodeProxyProgram_configs上面
    '''
    global need_rollback_Alice
    need_rollback_Alice = True
    for config in configs[1:]:  # configs第一个是给客户端的
        create_nodeProxyProgram(config)


def destroy_nodeProxyPrograms(configs):
    for config in configs:
        destroy_nodeProxyProgram(config)
...
def chooseNode(link: list, country: int, exit_ip,country_ips):
    '''
    选择一个节点
    link里包含前驱节点
    country: 选取节点的范围。对应country_list里的值
    '''
    selected_node = {i[0] for i in link}  # 已选的节点ip集合
    # 把出口ip提前算进已选节点
    ...
    link.append([random.choice(ip_pool), '-1', 'need cipher!', 'need key!'])
...
def freePort(ip: str, port: str):
    ...
    # 异常可能为port_dict里找不到ip这个键，或port_dict[ip]这个集合里找不到这个port
        ...
        print(f'Warning: 删除已使用端口时出错。{ip}无已使用的{port}端口；或ip不存在')
    else:
...
def createLink(length: int, ip_or_country, targetType: int):
    ...
    ip_or_country: 出口节点的ip（str）或国家（int）
    targetType: 为0表明ip_or_country表示的是ip，为1表明ip_or_country表示的是国家名称
    return: 返回描述链路的列表，link格式如下：
        link=[
            [ip:str, port:str, cipher:str, key:str],
            ...
        ]
    '''
    link = []
    ...
    for i in range(length - 1):
        chooseNode(link, country_list['中国'], exit_ip,country_ips)

    # 选取最后一个节点
    if isinstance(ip_or_country, str):  # 是ip
        link.append([ip_or_country, '-1', 'need cipher!', 'need key!'])
    elif isinstance(ip_or_country, int):  # 是country
        chooseNode(link, ip_or_country, exit_ip,country_ips)
    ...
    try:
        # 从每个节点处申请一个空闲的端口号，并随机生成加密方法和加密密码
        for i in range(length):
            link[i][1] = mallocPort(link[i][0])
            link[i][2] = getRandomBobMethod()
            link[i][3] = getRandomPassword(10)
            # 记录在数据库里
            insert_port_dict_database(link[i][0], link[i][1], link[i][3], link[i][2])
    except Exception as e:
...
def freeLink(link):
    ...
    for i in range(len(link)):
        freePort(link[i][0], link[i][1])


def generateSSHConfig(ip,node_configs):
    '''
    生成SSH的配置列表。
    '''
    ...
    return [
        ip,
        node_configs[ip][0],  # ssh端口
        node_configs[ip][1],  # ssh用户名
        node_configs[ip][2],  # ssh密码
    ]


def generate_nodeProxyProgram_deleting_config(link):
    """
    获取链路删除时的配置，即ssh（4个值）+该节点在该链路的端口
    """
    ret = []
    node_configs=read_node_configs()
    for node in link:
        ip = node[0]
        port = node[1]
        config = generateSSHConfig(ip,node_configs)
        config.extend(port)
        ret.append(config)
    return ret
...
#     ]
# ]
def generate_nodeProxyProgram_configs(link):
    '''
    生成需要为link链路下发到每个节点的程序运行命令，包含ssh连接参数和nodeProxyProgram运行参数
    '''
    configs = []
    node_configs=read_node_configs()
    for i in range(len(link)):
        now_ip = link[i][0]
        next_ip = link[i + 1][0] if i < len(link)-1 else None
        config = generateSSHConfig(now_ip,node_configs)
        config.extend([
            link[i][1],  # 本节点端口
            link[i][2],  # 本节点加密方式
            link[i][3],  # 本节点密码
            next_ip,  # 下一节点ip
            link[i + 1][1] if next_ip is not None else None,  # 下一节点端口
            link[i + 1][2] if next_ip is not None else None,  # 下一节点加密方式
            link[i + 1][3] if next_ip is not None else None,  # 下一节点密码
    ...
    configs.insert(0, {
        'rip': link[0][0],  # 入口节点ip
        'rport': link[0][1],  # 入口节点的端口号
        'rcipher': link[0][2],  # 入口节点的加密方式
        'rkey': link[0][3]  # 入口节点的密码
    })
    return configs
...
def rollback_for_buildLink0(link):
    ...
    if need_rollback_Alice:
        destroy_nodeProxyPrograms(generate_nodeProxyProgram_deleting_config(link))
...
def buildLink(username: str, linkLength: int, targetType: int, target: str):
    ...
    try:
        ...
        link_with_client_config = buildLink0(linkLength, targetType, target)
        link = link_with_client_config['link']
        client_config = link_with_client_config['client_config']
        ...
        # 写入链路
        userlinks.append(link_with_client_config)
        if not os.path.exists('userlink'):
            os.makedirs('userlink')
...
def buildLink0(linkLength: int, targetType: int, target: str):
    ...
    try:
        ...
        if targetType == 0:
            ...
            raise ShowToUserException('targetType的值不为0或1')

        # 生成链路，并让每个节点开启proxyProgram
        ...
        configs = generate_nodeProxyProgram_configs(link)
        create_nodeProxyPrograms(configs)

        write_port_dict()
        return {'client_config': configs[0], 'link': link}
    except Exception as e:
...
def deleteLink(username: str, link_to_del=None):
    '''
    删除指定用户的链路。
    返回成功删除的数量
    link_to_del是后加的参数，支持仅删除一条用户的链路。该值是client_config，也就是getLink的返回值（只有第一跳的配置信息，是一个字典）
    '''
    userLinks = None
    linkId = None
    current_link_to_del = None
    ret = 0
    try:
        if ' ' in username:
            raise ShowToUserException('参数错误：用户名不应含有空格')
        ...
        if link_to_del is None:
            # 删除该用户所有的链路
            for linkId in range(len(userLinks)):
                link = userLinks[linkId]['link']
                deleteLink0(link)
                ret += 1
            os.remove(f'./userlink/{username}.json')
        else:
            # 删除links_that_remain中的链路
            links_that_remain = []
            for userlink in userLinks:
                client_config = userlink['client_config']
                link = userlink['link']
                if client_config == link_to_del:
                    current_link_to_del = link
                    deleteLink0(link)
                    ret += 1
                else:
                    links_that_remain.append(userlink)
            userLinks = links_that_remain
            with open(f'./userlink/{username}.json', 'w') as file:
                json.dump(userLinks, file)
    except Exception as e:
        print(traceback.format_exc())
        if link_to_del is None:
            ...
            print('已成功删除的链路：', userLinks[:linkId])

            # 写回链路
            userLinks = userLinks[linkId:]
            with open(f'./userlink/{username}.json', 'w') as file:
                json.dump(userLinks, file)
            ...
            print('删除失败的链路为：', current_link_to_del)
...
def deleteLink0(link: list):
    ...
    for node in link:
        ip = node[0]
        port = node[1]
        # TODO 考虑数据库中已经没有该节点的记录。下面就有一个实现（但不是最新版的），需要预先传入失效的节点
        if ip in unlived_node_set:
            freePort(ip, port)
            delete_port_dict_database(ip, port)
            continue
        config = generateSSHConfig(ip,node_configs)
        config.append(port)
        destroy_nodeProxyProgram(config)
...
def insert_port_dict_database(ip: str, port: str, key: str, cipher: str):
    '''
    Bob服务开启成功后，写数据库
    '''
    try:
...
def delete_port_dict_database(ip: str, port: str):
    '''
    Bob服务开启成功后，写数据库
    '''
    try:
        ...
        cursor.execute('DELETE FROM port_dict WHERE ip=%s AND port=%s', (ip, port))
